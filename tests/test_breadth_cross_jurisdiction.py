...
def test_it_es_tape_path_cells_run_on_a_pool_declared_synthetic(matrix, deal_id):
    # These deals still publish no loan tape. Until #484 that made the
    # tape-path capabilities genuinely not-applicable; now each carries a
    # synthetic pool fitted to its own investor report, so they run — and the
    # #193 discipline moves from the *state* to the *reason*, which must carry
    # the source kind's own disclosure rather than reading like a filed tape.
    for cap in ("tape_analytics", "collateral_reconciliation"):
        cell = _matrix_cell(matrix, deal_id, cap)
        assert cell.state == STATE_RAN, f"{deal_id}/{cap}: {cell.state}"
        assert "Generated by LoanWhiz" in cell.reason, cell.reason
        assert "no figure computed from this tape is evidence about a real pool" in cell.reason, cell.reason
