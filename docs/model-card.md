...
RMBS deals use (#457), and no ground truth is authored for it, so it contributes
no validation figure below. Unvalidated here means no answer key has been
authored — a deferred decision — not that its published actuals are unobtainable. Extraction quality
