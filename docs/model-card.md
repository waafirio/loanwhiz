...
- **Cross-deal generalisation without validation.** The pipeline *runs* on every registered deal — `GET /capability-matrix` reports `ran` for tape analytics, covenant monitoring and waterfall execution on all of them, the Irish CLOs included, each of which executes through the engine kernel and is validated against nothing. The downstream waterfall engine is validated to the cent against external published actuals only on the Dutch **Green Lion** vintages, **2024-1** and **2023-1**. (Until #602 this bullet named 2024-1 alone: 2023-1's cell has read `validated` since #492 derived it from the committed answer key, so the matrix said one thing and this document another. Both are also graded to the cent by `GET /quality-matrix` against their own committed keys; a CLO's committed key grades published coverage-test outcomes there, which is a weaker claim than a to-the-cent reconciliation.) Extracted outputs on every other deal must be treated as unvalidated until independently verified — the Italian and Spanish prospectuses now extract at high *coverage* (0.925), but neither publishes a Notes & Cash report, so no published actuals exist to check them against.
...
The pipeline has been **run across every registered deal** (Dutch / Italian
/ Spanish RMBS — see [docs/data-card.md](data-card.md)). The Irish CLO Cairn CLO XVII DAC is extracted and **executes**, but is **not
validated**: its committed model folds through the same `run_period` kernel the
RMBS deals use (#457), and the ground truth authored for it (#481) is its trustee
reports' published coverage-test outcomes, not a Priority of Payments — so it
contributes a graded covenants row and no validation figure below. **A
published-PoP reconciliation now *is* performed** — #495 authored the ground
truth from its Note Valuation Report and, since epic #510, all 29 Interest steps
agree — so unvalidated here means the narrower thing: no validation builder is
committed, no offline engine series earns the `validated` cell, and 6 of those 29
lines are engine-computed while 23 are report-supplied (Limitation 1). It has
never meant that its published actuals are unobtainable. Extraction quality
...
The same extractor runs unchanged on every registered deal. Completeness is
...
| Cairn CLO XVII DAC | Ireland | **1.0** | Full 8-class stack, Interest / Principal / Post-Acceleration cascades, 10 triggers. Extracted, one row graded, **not validated** — its committed answer key (#481) carries the trustee reports' published coverage-test outcomes and no Priority of Payments, so `GET /quality-matrix` grading covers the covenants row only and no validation builder exists. Separately, its loan tape is *derived* from trustee reports by a deterministic parser rather than extracted by the model (see above) |
...
validated". The validated cells are the Green Lion vintages' engines against
their own published Notes & Cash reports, and how many there are is read live
from `GET /capability-matrix` rather than transcribed here — since #492 that
state is earned by committed answer keys, so the count moves whenever a key
lands. **Higher completeness on the non-English deals is
...
1. **Coverage is broad; external validation is not.** The pipeline runs on every registered deal — Dutch, Italian and Spanish RMBS plus the Irish CLOs — and extraction completeness is now 0.75–1.0 across all of them, but only the Dutch **Green Lion** vintages (2024-1 and 2023-1) are externally validated — each engine reconciled to the cent against that deal's own published Notes & Cash Priority of Payments, and each also graded by `GET /quality-matrix` against its committed answer key (#429/#440). Since #492 the `validated` cell is derived from those committed keys rather than from a hand-built builder map, which is what brought 2023-1's existing to-the-cent grade onto the capability matrix. The Italian and Spanish deals publish no Notes & Cash report, so they cannot be graded at all without inventing ground truth, and none is invented — since #484 they serve ledgers built on **synthetic pools fitted to their own published investor reports**, which is generated data carrying a `synthetic` provenance qualifier: it makes the deals modelable, and it is never ground truth and never a graded claim. The Irish CLO is extracted and executes — its own Interest and Principal cascades run through the shared engine over its full eight-class stack — and since #481 carries an answer key built from its trustee reports' published coverage-test outcomes, so `GET /quality-matrix` grades its covenants row. That is the narrowest of the graded claims and should be read as such: the compared ratio and threshold are both published figures, so it shows the monitor reaches the trustee's verdict, not that the engine reproduces the trustee's arithmetic. **Its Priority-of-Payments ground truth is authored (#495), the engine has been graded against it, and since epic #510 the Interest cascade reconciles.** When #496 first ran that grade it did not: the cascade distributed EUR 5,434,911.93 of the report's stated EUR 7,255,062.35, because 20 of the report's 62 published rows were attributed to no cascade step — EUR 1,820,150.42 the join had to recover — and not one Cairn line was computed by the engine, so every step that matched compared the report against itself. Each mechanism was closed separately: the join (#514), Class B's recipient-to-tranche seam (#538), the accrual period (#528), and each class's day-count basis (#539). Re-measured on the merged tree, the January 2025 period agrees on all 29 Interest steps at the key's EUR 0.01 tolerance, every published row reaches a step, and the engine's distribution ties to the report's stated pot at EUR 0.00. **Read the count rather than the verdict: 6 of those 29 lines are engine-computed and 23 are report-supplied.** The engine derives every one of the deal's interest-bearing classes — A through F — from the seed's tranche balance x the report's published applied rate x a day count taken from the prospectus Payment Date schedule and Conditions 6(e)(ii)/(iii); every other step's amount is taken from the report, so those 23 remain comparisons of the report with itself. The count read 3 until #598, when membership stopped being an authored list that ended at `class_c_interest` and started being derived from the need contract: Classes D, E and F had been reproducing their published interest to the cent all along and grading `report-supplied` anyway. 12 of the 29 carry money at all — the other 17 are EUR 0.00 against EUR 0.00 — and the 23-step Principal cascade reconciles on an EUR 0.00 pot, which an engine that never paid anything would reproduce exactly. The CLO is therefore **executed and graded, and still not validated**; the data card states what `validated` would and would not mean for a deal of this shape. CMBS, US RMBS and ABS are absent entirely. The capability matrix — read live from `GET /capability-matrix`, never transcribed — is the honest source of truth; never read the coverage as "validated across all deals".
