...
cascade now agrees and `engine_computed_passed` reads 6 (3 until #598 derived
the declaration; see the credited-classes note below).
...
computable, correctly-counted need, so all 29 Interest steps now agree and six
are independently computed (#511/#512/#528/#538/#539/#598). The cell would not have
...
deal publishes, 52 steps are compared and **6 are engine-computed; 46 are
report-supplied**. Every one of the 6 is a note-interest line — Classes A
through F, every interest-bearing class the deal issues — derived from the seed's
tranche balance, the report's published applied rate and a day count read off the
prospectus Payment Date schedule and Conditions 6(e)(ii)/(iii). The remaining 46
have their amount taken from the
...
any formula in the Listing Particulars. On this period 23 of the 46 sit in the
...
So the reconciliation's whole independent signal is 6 lines carrying EUR
5,911,043.33 of the Interest cascade's EUR 7,255,062.35 pot.

**That figure read 3 until #598, and the missing 3 were a fact about this repo
rather than about the document.** The engine computed a note-interest need only
for recipients named in an authored frozenset in
`primitives/step_source_classifier`, which stopped at `class_c_interest` — a set
written for a three-tranche RMBS stack. Cairn's Classes D, E and F reached the
fold with everything those three had: a seeded balance, an applied rate in the
same published `Rate Current` column, and a day-count basis parsed from Condition
6(e)(ii). Each reproduced its published interest exactly — EUR 594,969.17,
EUR 484,208.67 and EUR 495,004.89 — and graded `report-supplied` regardless.
#598 removed the list rather than extending it: membership is now derived from
`domain.rules.RECIPIENT_NEED_SOURCE`, so a recipient is credited when the
contract says the registry computes its need, and a class added to the stack is
credited without anyone editing a name list.

**Six and not ten, and the four left out are the more interesting half.** Four
further Interest steps — the Class C–F deferred-interest lines — and the two
management fees also name recipients with a registered calculator, and none of
them is credited, because a registered calculator is not the same thing as a
computed figure. The fees read a `fee_rates_pct` map and the deferred lines a
`deferred_interest_balance` field that the engine's own funds-builder never
populates. The fees at least refuse out loud; the deferred lines return a
...
|---|---|
| **Validation is far narrower than coverage** | The pipeline *runs* on every registered deal, the CLOs included; how many cells that leaves `validated` is reported by `GET /capability-matrix` and is deliberately not transcribed here (#602). **Green Lion 2024-1** and **Green Lion 2023-1** are validated to the cent against their own published Notes & Cash reports, and both are Dutch RMBS. Since #492 that cell is earned by committed data (an answer key carrying a Priority-of-Payments section, plus an offline engine series) rather than by bespoke Python, which is what made 2023-1's long-standing to-the-cent grade legible as validation. **Cairn CLO XVII** is additionally graded by `GET /quality-matrix` on its published coverage-test outcomes (#481) — a different check, against a different kind of document, and not a to-the-cent reconciliation. Its to-the-cent reconciliation was run separately (#496) and **fails**, which is why it is not among the validated deals. Every other cell is `ran` or `not-applicable` — outputs there are unvalidated and do not generalise without re-validation, and no non-Dutch and no non-RMBS deal is validated at all. |
...
| **Every registered jurisdiction runs; only the Dutch RMBS is validated** | The pipeline runs on Dutch, Italian and Spanish RMBS **and** on the Irish CLOs: `GET /capability-matrix` reports `ran` for tape analytics, covenant monitoring and waterfall execution on every registered deal, the CLOs included. This row previously said Ireland was registered but that nothing had been run against it — the matrix contradicted it, and it was corrected against the matrix rather than by hand (#602). Running is not validating: the CLOs reconcile against no published Notes & Cash report, and outside the Dutch RMBS no cell is `validated`. Coverage of other European or non-European markets is untested. |
