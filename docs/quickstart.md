...
- `GET /capability-matrix` — the primitives × 6 registered deals capability matrix: each cell `validated` / `ran` / `not-applicable` with a real reason, plus the tally (**1 validated / 15 ran / 14 not-applicable**)
