...
- `GET /capability-matrix` — the primitives × 6 registered deals capability matrix: each cell `validated` / `ran` / `not-applicable` with a real reason, plus the tally (**2 validated / 15 ran / 13 not-applicable**)
