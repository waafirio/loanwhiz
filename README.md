...
The same governed primitives run **end-to-end across 5 deals in 3 jurisdictions** — Dutch (Green Lion 2023-1 / 2024-1 / 2026-1), Italian (Leone Arancio RMBS 2023-1), and Spanish (Sol-Lion II RMBS). A sixth deal, the Irish CLO **Cairn CLO XVII DAC**, is now **extracted but not validated** (#456): the pipeline reads its 420-page Listing Particulars to a full 8-class capital stack, both the Interest and Principal Priorities of Payments, and its per-class coverage tests — but no answer key is authored, so no cell of it is `validated`. And the model-driven waterfall engine has been **validated to the cent against a real published deal** (Green Lion 2024-1's own Notes & Cash Priority of Payments). What is *validated* vs merely *ran* vs *not-applicable* is tracked honestly in a per-cell capability matrix (`GET /capability-matrix` and the **Showcase** view) — the source of truth is **1 validated / 14 ran / 15 not-applicable**, never a blanket "validated everywhere". A second deal (Green Lion 2023-1) now has a committed ground-truth answer key, so `GET /quality-matrix` **grades two deals** to the cent; extraction *coverage* on the non-English prospectuses is no longer thin, but coverage is not validation (see the data/model cards). The primitives are also packaged as a governed **MCP server** (`mcp/`) for third-party consumption.
...
CROSS-DEAL / FRAMEWORK SURFACE
      capability matrix  (primitives x 6 registered deals, validated/ran/not-applicable; /capability-matrix + Showcase)
...
6. **Showcase** — the primitives × 6 registered deals **capability matrix** (Dutch / Italian / Spanish RMBS, plus the extracted-but-unvalidated Irish CLO), each cell `validated` / `ran` / `not-applicable` with the honest reason behind it (tally **1 validated / 14 ran / 15 not-applicable**).
...
>   `validated` / `ran` / `not-applicable` — currently **1 validated / 14 ran /
...
**The full deal set — 6 registered deals, 5 that run.** Alongside 2026-1, the registry (`src/loanwhiz/data/deals.json`) carries four more deals the *same* primitives run on end-to-end: the seasoned Dutch deals **Green Lion 2023-1** and **2024-1** (the latter is the engine's to-the-cent validation target), the Italian **Leone Arancio RMBS 2023-1 S.r.l.**, and the Spanish **Sol-Lion II RMBS Fondo de Titulización**. Extraction completeness is honest per deal and, after the #438/#439 re-extractions, is **0.925 on both the Italian and the Spanish** prospectuses — comparable to the Dutch deals. That is *coverage*: neither publishes a Notes & Cash report, so neither can be externally validated. A sixth deal — the Irish CLO **Cairn CLO XVII DAC** — is **extracted but not validated** (#455 sourced it, #456 extracted it): its 420pp Listing Particulars, three U.S. Bank monthly trustee reports and 83pp Note Valuation Report are all freely obtainable from Euronext Dublin. The pipeline now reads the Listing Particulars to a full 8-class stack (Class A through F plus the unlettered Subordinated Notes, EUR 404.1m), both Priorities of Payments as distinct cascades, and 10 triggers of which 8 are per-class coverage tests. Its `covenant_monitoring` and `waterfall_execution` cells are `ran`; no loan tape exists in a readable format, no answer key is authored, and nothing about it is `validated`. The capability matrix (`/capability-matrix`, Showcase view) is the per-deal source of truth. See [docs/data-card.md](docs/data-card.md) — whose CLO subsection records what is and is not obtainable, negatives included — and [docs/model-card.md](docs/model-card.md) for the full per-deal breakdown.
