...
**The full deal set — what is registered, and what runs.** Alongside 2026-1, the registry (`src/loanwhiz/data/deals.json`) carries four more deals the *same* primitives run on end-to-end: the seasoned Dutch deals **Green Lion 2023-1** and **2024-1** (the latter is the engine's to-the-cent validation target), the Italian **Leone Arancio RMBS 2023-1 S.r.l.**, and the Spanish **Sol-Lion II RMBS Fondo de Titulización**. Extraction completeness is honest per deal and, after the #438/#439 re-extractions, is **0.925 on both the Italian and the Spanish** prospectuses — comparable to the Dutch deals. That is *coverage*: neither publishes a Notes & Cash report, so neither can be externally validated. A sixth deal — the Irish CLO **Cairn CLO XVII DAC** — is **extracted but not validated** (#455 sourced it, #456 extracted it): its 420pp Listing Particulars, three U.S. Bank monthly trustee reports and 83pp Note Valuation Report are all freely obtainable from Euronext Dublin. The pipeline now reads the Listing Particulars to a full 8-class stack (Class A through F plus the unlettered Subordinated Notes, EUR 404.1m), both Priorities of Payments as distinct cascades, and 10 triggers of which 8 are per-class coverage tests. Its `covenant_monitoring` and `waterfall_execution` cells are `ran`; no loan tape exists in a readable format, no answer key is authored, and nothing about it is `validated`. A **second** Irish CLO — **Contego CLO XI DAC** (Five Arrows, BNY Mellon) — was registered and extracted by #532 (completeness 0.85; the full 8-class **2023** stack, EUR 380.6m); it exists to test whether the CLO capability is general or fitted to Cairn, changing exactly one variable, the collateral administrator. It is registered against its **29-Jun-2023** Listing Particulars rather than its 19-Nov-2024 **reset**, because every trustee report published for it predates the reset — pairing the reports with the newer document would have made every downstream figure confidently wrong rather than visibly broken. The capability matrix (`/capability-matrix`, Showcase view) is the per-deal source of truth. See [docs/data-card.md](docs/data-card.md) — whose two CLO subsections record what is and is not obtainable, negatives and the reset boundary included — and [docs/model-card.md](docs/model-card.md) for the full per-deal breakdown.
