 ...
 * rows were generated by LoanWhiz and describe no real obligor (issue #483).
 *
 * A label says how the tape arrived, not what it is. What it is — and in
 * particular that it is **not** an Article 7(1)(a) regulatory filing, or that
 * nothing computed from it is evidence about a real pool — is carried in full
 * in the citation excerpt, which quotes the backend's own disclosure sentence.
 * Rendering this label alone is not a substitute for showing that sentence.
 */
...
export function citationDataSource(c: Citation): DataSource | null {
  ...
  const m = /ingested via (deeploans|direct|derived|synthetic)/i.exec(
    c.excerpt ?? "",
  );
